...
&此时对比题目要求的结果是x+3y。\\
&思考：如何才能让 x+2y+\dfrac{x+y}{2}（通过某种变换）\rightarrow\quad x+3y?\\
&显然\dfrac{x+y}{2}中，分子x，y前面有某种系数就可以。\\
&但是的x+y是由\sqrt{xy}得来的，这是题目给出的条件，我们无法修改，我们应该如何做？\\
&没办法改变条件，但可以对条件进行变形，利用a\times\dfrac{1}{a}的特性，把\sqrt{xy}\rightarrow\sqrt{(a\cdot x) \times\dfrac{y}{a}}\\
&由基本不等式得：\sqrt{(ax) \times\dfrac{y}{a}}\le\dfrac{ax+\dfrac{y}{a}}{2}\\
&现在的问题就变成如何求出a的值。\\
&原式：2\le x+2y +\dfrac{ax+\dfrac{y}{a}}{2}=(\dfrac{a}{2}+1)x+(2+\dfrac{1}{2a})y\\
